...
echo "==> 准备运行目录与配置..."
mkdir -p "$JAR_DIR" "$CONF_DIR"
chown -R www-data:www-data "$JAR_DIR"
chmod 755 "$CONF_DIR"
if [[ ! -f "$CONF_DIR/application-prod.yml" ]]; then
  ...
  echo "已生成 $CONF_DIR/application-prod.yml ，请编辑数据库 / JWT / OAuth 后再启动"
fi
...
echo "=========================================="
echo "  后端部署完成"
echo "  配置文件: $CONF_DIR/application-prod.yml"
...
echo "  2) 编辑 $CONF_DIR/application-prod.yml"
