...
echo "=========================================="
echo "  记得:"
echo "  1) 导入 MySQL: schema.sql / data.sql（及 migration 如需）"
echo "  2) 编辑 $CONF_DIR/backend.env（含 BLOG_STORAGE_ROOT，勿提交真实密钥）"
echo "  3) Nginx 反代 /api/ 与 /uploads/ （见 frontend/deploy/nginx.conf.example）"
echo "  4) NAS 对象存储: $APP_DIR/backend/deploy/nas-storage.zh-CN.md"
echo "     English:     $APP_DIR/backend/deploy/nas-storage.md"
echo "  5) 挂载健康检查示例:"
echo "     findmnt /mnt/nas-blog && sudo -u www-data test -w /mnt/nas-blog"
echo "=========================================="
